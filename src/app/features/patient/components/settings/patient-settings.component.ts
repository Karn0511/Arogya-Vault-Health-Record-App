...
export class PatientSettingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.load();
    // connect real-time socket to receive updates (non-blocking)
    try {
      this.socketService.connect();
      this.socketService.on('notifications:change').subscribe((change: any) => {
        // If a notification for current user indicates activity, refresh snapshot
        try {
          const profileId = (this.profile as any)?._id ?? (this.profile as any)?.id;
          if (change && profileId && change.fullDocument && change.fullDocument.userId === profileId) {
            this.load();
          }
        } catch (e) {
          // ignore any runtime mismatch while parsing change
        }
      });
    } catch (e) {
      // ignore if socket cannot connect in this environment
    }
  }
