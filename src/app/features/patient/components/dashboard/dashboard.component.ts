...
export class DashboardComponent implements OnInit {
  ...
  private loadDashboardData(): void {
    ...
    // Load appointments
    this.http.get<any>(`${environment.apiUrl}/appointments`, { headers })
      .subscribe({
        next: (response) => {
          ...
          const futureAppts = appointments.filter((apt: any) => {
            const aptDate = new Date(apt.date || apt.appointmentDateTime || apt.appointmentDate);
            const patientId = apt.patientId?._id || apt.patientId?.id || apt.patientId;
            return aptDate >= new Date() && patientId === userId;
          }).sort((a: any, b: any) => new Date(a.date || a.appointmentDateTime || a.appointmentDate).getTime() - new Date(b.date || b.appointmentDateTime || b.appointmentDate).getTime());

          this.upcomingAppointments = futureAppts.slice(0, 5).map((apt: any) => ({
            doctor: apt.doctorId?.fullName || apt.doctorId?.name || 'Dr. Unknown',
            specialization: apt.reason || 'Check-up',
            date: apt.appointmentDate ? new Date(apt.appointmentDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : 'TBD'
          }));
    ...
    // Load recent medical reports (replacing generic patient-health call)
    this.http.get<any>(`${environment.apiUrl}/medical-reports`, { headers })
      .subscribe({
        next: (response) => {
          const records = Array.isArray(response) ? response : response.data || [];
          this.recentRecords = records.slice(0, 5).map((rec: any) => ({
            id: rec._id,
            name: rec.testName || rec.recordName || 'Health Record',
            type: rec.type?.toLowerCase() || 'other',
            date: rec.testDate ? new Date(rec.testDate).toLocaleDateString() : new Date().toLocaleDateString()
          }));
    ...
    this.http.get<any>(`${environment.apiUrl}/prescriptions`, { headers })
      .subscribe({
        next: (response) => {
    ...
    // Load vaccinations
    this.http.get<any>(`${environment.apiUrl}/vaccinations`, { headers })
      .subscribe({
        next: (response) => {
