        ...
        try {
            ...
            if (process.env.AWS_ACCESS_KEY_ID && process.env.AWS_S3_BUCKET) {
                try {
                    ...
                    fileUrl = s3Result.fileUrl; // Use S3 URL
                    s3Key = s3Result.s3Key;

                    // Remove local file after S3 upload
                    fs.unlink(req.file.path, (err) => {
                        if (err) console.error('Error deleting local file after S3 upload:', err);
                    });
                } catch (s3Error) {
                    console.error('S3 Upload failed, falling back to local:', s3Error);
                    // Continue with local fileUrl
                }
            }

            const fileStorage = new FileStorage({
                patientId: req.userData.id,
                userId: req.userData.id, // Support generic userId field too
